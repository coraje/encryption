	...
	<div class="combine">
		...
		<div class="paging">
			<%if (nowpage <= 1) {%><a><span class="prev"></span></a>
				...
				<%} else {%><a href="./reviewList.do?page=<%=nowpage - 1%>"><span class="prev"></span></a><%}%>
				
				<%for (int a = startpage; a <= endpage; a++) {if (a == nowpage) {%><a class="current"><%=a%></a>
				
				<%} else {%><a href="./reviewList.do?page=<%=a%>"><%=a%> </a><%}%><%}%>
				
				<%if (nowpage >= maxpage) {%><a><span class="next"></span></a>
				
				<%} else {%><a href="./reviewList.do?page=<%=nowpage + 1%>"><span class="next"></span></a></li><%}%>
		</div>
	</div>
</div>
<!-- //container -->